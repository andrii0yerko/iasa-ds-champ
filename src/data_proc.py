...
def generate_date_features(df):
    df[["install_year", "install_month", "install_day"]] = (
        df["install_date"].astype(str).str.split("-", expand=True).astype(int)
    )

    cyclical = CyclicalFeatures(variables=None, drop_original=True)
    sin_cos_features = cyclical.fit_transform(
        df[["install_day", "install_month", "install_year"]]
    )
    ...
    df["installed_on_holiday"] = df["install_date"].apply(
        lambda x: False if x in non_holidays else True
    )

    install_date_users = df["install_date"].value_counts()
    df["users_joined_this_day"] = df["install_date"].apply(
        lambda x: install_date_users[x]
    )
    ...
    def count_difference(cols, days, interval=1):
        for col in cols:
            for i in range(len(days) - 1, interval - 1, -interval):
                df[col + str(days[i]) + "_delta"] = (
                    df[col + str(days[i])] - df[col + str(days[i - interval])]
                )

    for cols, days in zip(
        [daily_cols, ret_cols, sessions_cols, trg_daily_cols],
        [daily_days, ret_days, sessions_days, trg_daily_days],
    ):
